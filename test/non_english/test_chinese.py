...
from bible_io_references.languages import chinese
...
@pytest.mark.parametrize(
    "reference, expected",
    [
        ("约翰福音 3:16", (BibleBookEnum.John, 3, 16)),
        ("啟 21:4", (BibleBookEnum.Revelation, 21, 4)),
    ],
)
...
def test_parse_chinese_book_names(expected_book):
    names = chinese.BOOK_NAMES.get(expected_book)
...
@pytest.mark.parametrize("expected_book", list(BibleBookEnum))
def test_parse_chinese_book_abbreviations(expected_book):
    abbreviations = chinese.BOOK_ABBREVIATIONS.get(expected_book)
