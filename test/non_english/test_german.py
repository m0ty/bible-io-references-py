...
@pytest.mark.parametrize(
    ...
    [(f"{names[0]} 1:1", book) for book, names in german.BOOK_NAMES.items()],
)
def test_parse_german_book_names(reference, expected_book):
    ref = references.VerseRef.from_str(reference)
...
@pytest.mark.parametrize(
    ...
    [
        (f"{abbr} 1:1", book)
        for book, abbreviations in german.BOOK_ABBREVIATIONS.items()
        for abbr in abbreviations
    ],
)
def test_parse_german_book_abbreviations(reference, expected_book):
    ref = references.VerseRef.from_str(reference)
